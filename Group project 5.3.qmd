...
## **Describing and Handling Missing Values**
...
# How should we handle missings in the responders data set? Should we eliminate participants x waves not present in a specific wave?
```
...
**correct with the new primary keys**

To determine units it is necessary to find the primary key of the data set. For both the responders and the debate dataset the primary key is 'Responder'. The code below shows that the values of the 'Responder' variable is unqiue in both datasets. This means that the value only appears once in each dataset. For the content analysis dataset, the primary key is *Fortlaufende*, which represents a time unit that has been classified as a sentence in the debate.
...
Comment on the primary keys. Why do we only need to split the *responders* data set?

**Splitting the data.**

**Principle 1: Each type of case needs its own tibble.**

Information on each different type of case in a data set must be transferred into a separate tibble.

Split data into different tables. The code below creates a table from the responders dataset that contains the responders' characteristics.
...
joined <- debate |> inner_join(content_analysis, by = "time")
...
The foreign key of the tidied *debate* data set is now `time` (in seconds). It is easy to ???
