...
#Visualization.
#basic plot
df_lx_clean |> mutate(Gender_rc = case_when(
  Gender == 1 ~ "Male",
  Gender == 2 ~ "Female"
)) |> 
ggplot(aes(x = time, y = score_avg_time, group = Gender_rc)) +
  geom_line(size = 0.5) +
  facet_wrap(~Gender_rc) +
  labs(title = "Average Change in Opinions Over Time",
       x = "Live Debate (in seconds)",
       y = "Opinions",
       color = "Merkel-Steinmeier Scale")
```
