...
The responders and debate tables have a many-to-many relationship. In the responder table, for every responder there are four rows/cases that each represent a wave. Therefore multiple units, reponder ID are related to multiple units in the debate table. Making this a many-to-many relationship.
...
To refresh your memory, research question two is "How do the opinions of the voters regarding the candidates change throughout the debate for different voter groups (by age, gender)?" This research question aims to explain differences in the changes in the responders' live reaction to the debate, by age and gender as group characteristics. To answer research question two, data from two tables are required. First, it requires the respondents' live reactions to the debate from the debate table. Second, it requires the respondents' characteristics from the respondent characteristics table.The dependent variable (y) is 'score', which measures the relative support for Merkel (0) and support for Steinmeier (6) on a 6-point Likert-scale. This variable was recoded from 1-7 to 0-6. The x-axis variable is 'time', which measures the time of the live debate in seconds, ranging from 0s - 5493s. The grouping variables are Age and Gender in which female (1), and male (2).
...
ggplot(aes(x = time, y = score_avg_time, group = as.factor(Gender_rc), color = as.factor(Gender_rc))) +
