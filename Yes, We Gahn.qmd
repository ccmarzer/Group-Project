...
### *Exploring the Most Boring Political Debate in History (Germany, 2009, Merkel versus Steinemeier)*
...
To get started, we need to install and load the packages required for our analysis. Packages are extensions to the R statistical programming language, necessary for several tasks we plan to carry out, such as data translation, wrangling, and visualization.
...
We can now import data on responders' political behaviour and live reactions during the TV duel, and on the debate's contents and dynamics.
...
The GLES 2009 data sets are only downloadable in German, and none of us speaks any German, so we need to translate at least the content analysis data to better understand the context of the debate and the topics and themes discussed during the televised duel.

We employ the automatic DeepL translator, using a DeepL Pro account. However, this option only allows for limited translation in any given month. Therefore, the code for translating the content analysis data set is de-activated, and provided in comments to make our report replicable.

```{r}
# Some column names from the responders data set are in German.

# With the rename() function from the "dplyr" package, we can manually translate them into English.
