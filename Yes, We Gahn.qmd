...
ggplot(aes(x = time, y = score_avg_time, group = as.factor(Gender_rc), color = as.factor(Gender_rc))) +
  ...
  geom_hline(yintercept = 3, linetype = "dashed", color = "chartreuse2", linewidth = 1.8) +
  scale_x_time() +
  facet_wrap(~Age_rc) +
  labs(title = "Voters' Opinion to Steinmeier versus Merkel",
       ...
       x = "Progression Live Debate (in H:M:S)",
       y = "Steinmeier-Merkel Scale",
       color = "Gender") +
  theme_bw() +
  theme(legend.position = "bottom",
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(size = 12, face = "italic", hjust = 0.5, margin=margin(0,0,15,0)),
    legend.text = element_text(size = 12),
    strip.background = element_rect(fill = c("darkseagreen1"), color = "black", linewidth = 0.7),
